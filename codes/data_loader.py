...
class DataLoader:
    ...
    def next(self, batch_size = config.BATCH_SIZE):
        ...
        for i in range(batch_size):
            ...
            X[i], Y[i] = \
                self.inputs[self.index], self.labels[self.index]
    ...
    def read_all_batch(self):
        ...
        self.inputs         = np.empty([self.size, config.IMAGE_FRAMES, config.IMAGE_HEIGHT, config.IMAGE_WIDTH, config.IMAGE_CHANNEL], dtype=np.uint8)
